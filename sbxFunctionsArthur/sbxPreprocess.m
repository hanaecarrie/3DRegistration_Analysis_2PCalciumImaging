function sbxPreprocess(mouse, date, varargin)
...
%   6. Save reference TIFFs
    ...
    addOptional(p, 'downsample_t', 5, @isnumeric);  % The number of frames to downsample for cell extraction
    ...
    if p.align_from_xyreg
        xyregpaths = {};
        for r = 1:length(runs)
            xyregpaths{end+1} = sbxPath(mouse, date, runs(r), 'xyreg', 'pmt', p.refpmt);
        end
        sbxpaths = xyregpaths;
    end
    if p.align_from_sbxreg
        sbxregpaths = {};
        for r = 1:length(runs)
            sbxregpaths{end+1} = sbxPath(mouse, date, runs(r), 'sbxreg', 'pmt', p.refpmt);
        end
        sbxpaths = sbxregpaths;
    end
    
    if p.align_stackreg
        sbxStackReg(sbxpaths, 'force', p.force, 'edges', p.refedges, 'pmt', p.refpmt, ...
            'target', p.target, 'refsize', p.refsize, 'refoffset', p.refoffset,'tbin', p.align_tbin_s,...
            'binxytarget', p.ref_downsample_xy, 'chunksize', chunksize, 'xrunpmt', p.xrunpmt);
    elseif p.align_demonsreg
        sbxDemonsReg(sbxpaths, 'force', p.force, 'edges', p.refedges, 'pmt', p.refpmt, ...
            'target', p.target, 'refsize', p.refsize, 'refoffset', p.refoffset,...
            'binxytarget', p.ref_downsample_xy, 'chunksize', chunksize);
    else
        sbxAlignAffineDFT(sbxpaths, 'force', p.force, 'edges', p.refedges, 'pmt', p.refpmt, ...
            'target', p.target, 'refsize', p.refsize, 'refoffset', p.refoffset, 'tbin', p.align_tbin_s, ...
            'highpass_sigma', p.highpass_sigma, 'chunksize', chunksize, 'aligntype', p.aligntype);
    end
    %% Align from pcacleaned
    if p.align_from_pcaclean
        pcapaths = {};
        for r = 1:length(runs)
            pcapaths{end+1} = sbxPath(mouse, date, runs(r), 'sbxclean');
        end 
        if ~isempty(pcapaths{1})

            if p.align_stackreg
                sbxStackReg(pcapaths, 'force', p.force, 'edges', p.refedges, 'pmt', p.refpmt, ...
                    'target', p.target, 'refsize', p.refsize, 'refoffset', p.refoffset,'xrunpmt', p.xrunpmt,...
                    'binxytarget', p.ref_downsample_xy, 'chunksize', chunksize,'save_title', 'pcacleaned');
            else
                sbxAlignAffineDFT(pcapaths, 'force', p.force, 'edges', p.refedges, 'pmt', p.refpmt, ...
                    'target', p.target, 'refsize', p.refsize, 'refoffset', p.refoffset, 'tbin', p.align_tbin_s, ...
                    'highpass_sigma', p.highpass_sigma, 'chunksize', chunksize,'save_title', 'pcacleaned', 'aligntype', p.aligntype);
            end
        end
    end
    ...
    for r = 1:length(sbxpaths)
        ...
        if p.align_from_xyreg
            path = sprintf('%sxyreg-%i.sbxreg', path(1:strfind(path,...
                ['xyreg-' num2str(p.refpmt) '.'])-1), p.pmt);
        end
        
        sbxSaveAlignedSBX(path, 'pmt', p.pmt,'stackregshifts',p.align_stackreg, 'demonsregshifts', p.align_demonsreg, 'refpmt',p.refpmt);
        
        if p.pmt ~= p.refpmt && p.save_refpmt
            if p.align_from_xyreg
                path = sbxpaths{r};
                path = sprintf('%sxyreg-%i.sbxreg', path(1:strfind(path,...
                ['xyreg-' num2str(p.refpmt) '.'])-1), p.refpmt);
            end
            sbxSaveAlignedSBX(path, 'pmt', p.refpmt,'stackregshifts',p.align_stackreg,'demonsregshifts', p.align_demonsreg,'refpmt',p.refpmt);
        end
        
        if p.align_stackreg
            runpath = sprintf('%s_xyreg-%i.sbxreg', path(1:strfind(path,'.')-1), p.pmt);
            pcapath = sprintf('%s_xyreg-%i.sbxclean', path(1:strfind(path,'.')-1), p.pmt);
        elseif p.pcaclean_demonsreg
            runpath = sprintf('%s_demonsreg-%i.sbxreg', path(1:strfind(path,'.')-1), p.pmt);
            pcapath = sprintf('%s_demonsreg-%i.sbxclean', path(1:strfind(path,'.')-1), p.pmt);
        else
            runpath = sprintf('%s_reg-%i.sbxreg', path(1:strfind(path,'.')-1), p.pmt);
            pcapath = sprintf('%s_reg-%i.sbxclean', path(1:strfind(path,'.')-1), p.pmt);
        end
        ...
        if p.align_from_pcaclean
            sbxSaveAlignedSBX(pcapath, 'pmt', p.pmt, 'save_title', 'pcacleaned','stackregshifts',p.align_stackreg,'refpmt',p.refpmt);
            if p.pmt ~= p.refpmt && p.save_refpmt
                sbxSaveAlignedSBX(pcapath, 'pmt', p.refpmt,'save_title', 'pcacleaned','stackregshifts',p.align_stackreg,'refpmt',p.refpmt);
            end
        end    
    ...
    if ~strcmpi(p.extraction, 'none') 
        ...
        processForJavascript(mouse, date, runs);
    end
