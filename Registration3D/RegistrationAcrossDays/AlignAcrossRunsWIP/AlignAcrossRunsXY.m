function [] = AlignAcrossRunsXY(inputfolders, mouse, date, runs,...
    BlurFactor, KeepingFactor, n, outputfolders, nbchunck, server)
...
tStart = tic;
info = sbxInfo(sbxPath(mouse, date, runs(1),'sbx', 'server', server));
zp = info.otparam(3);
...
idx = 2;
 for run = runs(2:end)
    ...
    saveVolumeRegistration(savingpath, volumeregacrossruns,...
        'volumeregacrossrunsXY', mouse, date, run, ...
        'nbchuncktiff', nbchunck);
